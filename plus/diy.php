...
if($action == 'post')
{
    ...
    elseif($do == 2)
    {
        ...
        if($dsql->ExecuteNoneQuery($query))
        {
			    ...
			    $email = "463263454@qq.com,1273258462@qq.com";  //这里填写要发送到的邮箱
				$mailtitle = "陕西清蓝生物科技有限公司dedecms系统内部测试邮件";
				$mailbody = "陕西清蓝生物科技有限公司dedecms系统内部测试邮件内容";
				sendmail($email, $mailtitle, $mailbody);
					...
					$mailtitle = "测试邮件标题";    //自行设置
					$mailbody = "测试邮件内容";    //自行设置
					$mailtype = "html";
					require_once(DEDEINC."/mail.class.php");
					$smtp = new smtp($cfg_smtp_server, $cfg_smtp_port, true, $cfg_smtp_usermail, $cfg_smtp_password);
					$smtp->debug = false;
